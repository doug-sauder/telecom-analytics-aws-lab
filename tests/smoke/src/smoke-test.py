...
PROMETHEUS_API_ENDPOINT = os.getenv("SMOKE_PROMETHEUS_API_ENDPOINT", "http://prometheus:9090/api/v1/query")
CHECK_PROMETHEUS = os.getenv("SMOKE_CHECK_PROMETHEUS", "true").lower() == "true"
INGEST_READY_URL = os.getenv("SMOKE_INGEST_READY_URL", "http://ingest:3000/readyz")
PMGEN_READY_URL = os.getenv("SMOKE_PMGEN_READY_URL", "http://pmgen:8000/readyz")
...
VALIDATE_PMGEN_FLOW = os.getenv("SMOKE_VALIDATE_PMGEN_FLOW", "false").lower() == "true"
PMGEN_SOURCE = os.getenv("SMOKE_PMGEN_SOURCE", "pmgen")
...
def check_http_ready_endpoint(name, url):
    # Verify that Kubernetes or Compose DNS resolves a service name and reaches its readiness endpoint.
    #
    # Parameters:
    #   name: Human-readable service name used in failure output.
    #   url: Readiness URL to call from the smoke-test container.
    try:
        response = requests.get(url, timeout=10)
    except requests.Timeout:
        print(f"Error: Connection to {name} timed out at {url}.")
        exit(1)
    except requests.ConnectionError:
        print(f"Error: Failed to connect to {name} at {url}.")
        exit(1)

    if response.status_code != 200:
        print(f"Error: {name} readiness check failed at {url}. Status code:", response.status_code)
        exit(1)

    print(f"{name} service discovery verified:", url)


def check_service_discovery():
    # Confirm application service DNS names resolve and route to healthy endpoints.
    check_http_ready_endpoint("ingest", INGEST_READY_URL)
    check_http_ready_endpoint("pmgen", PMGEN_READY_URL)
...
def count_pmgen_events_since(start_time):
    # Count fresh events generated by the deployed pmgen service.
    query = """
        SELECT count(*)
        FROM analytics.pm_events
        WHERE source = %s
    ...
    with psycopg.connect(**postgres_connection_kwargs()) as connection:
        with connection.cursor() as cursor:
            cursor.execute(query, (PMGEN_SOURCE, start_time))
            row = cursor.fetchone()

    return row[0]


def wait_for_pmgen_event_in_postgres(start_time):
    # Poll for an event produced by the running pmgen deployment after validation started.
    deadline = time.monotonic() + POSTGRES_POLL_TIMEOUT_SECONDS
    last_error = None

    while time.monotonic() < deadline:
        try:
            event_count = count_pmgen_events_since(start_time)
        except psycopg.Error as error:
            last_error = str(error)
            event_count = 0

        if event_count > 0:
            print("pmgen event flow verified. Fresh pmgen events stored:", event_count)
            return

        time.sleep(POSTGRES_POLL_INTERVAL_SECONDS)

    if last_error:
        print("Error: Failed to query Postgres for pmgen events:", last_error)
    else:
        print("Error: No fresh pmgen events were stored in Postgres.")
    exit(1)
...
def main():
    ...
    if VALIDATE_PMGEN_FLOW:
        wait_for_pmgen_event_in_postgres(validation_start_time)
    print("Test complete")
