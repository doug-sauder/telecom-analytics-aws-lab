...
| Namespace | Component | Workload | Services | Persistent data |
| --- | --- | --- | --- | --- |
| `data-services` | PostgreSQL | `StatefulSet/postgres` | `postgres`, `postgres-headless` | StatefulSet PVC |
| `data-services` | Redpanda | `StatefulSet/redpanda`; initialization Job | `redpanda`, `redpanda-internal` | StatefulSet PVC |
| `telecom-analytics` | Ingest | `Deployment/ingest` | `ingest` | - |
| `telecom-analytics` | PM generator | `Deployment/pmgen` | `pmgen` | - |
| `monitoring` | Prometheus | `Deployment/prometheus` | `prometheus` | `prometheus-data` |
| `monitoring` | PostgreSQL exporter | `Deployment/postgres-exporter` | `postgres-exporter` | - |
| `monitoring` | Grafana | `Deployment/grafana` | `grafana` | `grafana-data` |

ConfigMaps and Secrets supporting these components are omitted. Pods and StatefulSet
PVCs are generated by their controllers.
